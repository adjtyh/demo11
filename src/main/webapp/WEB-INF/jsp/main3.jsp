		...
		<div class="row clearfix">
			<div class="col-md-2 column">
				<ul class="nav nav-pills nav-stacked ">
					...
					<li role="presentation" class="active"><a href="http://localhost:8080/demo11/main2.html">报表3</a></li>
				</ul>
			</div>
			<div class="col-md-10 column">
				<div class="row clearfix">
					<div class="col-md-8 column">
						<form class="form-inline" name="form1" action="doMain3.html"
							method="post">
				...
				<div class="row clearfix">
					<!-- <table class="table table-bordered"> -->
						<table border="1">
						<form action="doLbzbb.html" method="post" name="form2"
							target="_blank">
							<caption>两病周报表<br/>
							注意：本表数据必须认真核对，没有的应填0；各数据要符合逻辑关系；在每周五填报当周数据，在当周星期天停止收集数据。
							</caption>
							<thead>
								<tr>
									<th>序号</th>
									...
									<c:forEach var="num2" begin="10" end="17" step="1">
										<th>i${num2 }</th>
									</c:forEach>
								</tr>
							</thead>
							<tbody>
								...
								<%
									PageParamLbzbb pageParamLbzbb = (PageParamLbzbb) request.getAttribute("pageParamLbzbb");
								%>
								<!--循环输出表格单元  -->
								<c:forEach items="${pageParamLbzbb.data }" var="item">

									<tr>
										<td>${item.lId }</td>
										<td><a href="lookLbzbb.html?lId=${item.lId }"
											target="_blank">查看 </a></td>
										<td><a href="updateLbzbb.html?lId=${item.lId }"
											target="_blank">修改</a></td>
										<td>
											<button type="button" class="btn btn-danger"
												onclick="del(${item.lId })">删除</button>
										</td>
							...
							<input id="dellId" name="dellId" type="hidden" value="0" />
						</form>
					</table>
					<p>返回行数：${rowCount }</p>
...
<script lang="javascript">
	...
	function del(lId) {
		if( confirm("确定要删除这条记录吗？")==false){
			...
			document.getElementById("dellId").value = lId; 
			form2.submit();
		}
	}
</script>
